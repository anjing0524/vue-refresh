...
| tests/core.test.ts、tests/vue.test.ts | 52 个现行契约自动测试通过；含真实 Vue/KeepAlive/Pinia 和两个 SSR 渲染。定制渲染器测试不冒充真实浏览器可见性测试 |
| pnpm build | 通过。生成 `dist/index.js`（21.82 kB / gzip 6.73 kB）与类型声明（构建后处理改写说明符为 `.js`，并断言产物形态）；按包导入仅得到 defineRefresh、createRefreshManager、useRefresh 三个函数 |
...
| pnpm complexity | 12 个源文件、1916 行、134 个结构分支、最大函数圈复杂度 8 |
