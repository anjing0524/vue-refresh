...
整段 `options` 也可以是它的 Ref / getter（`RefreshInput<RefreshOptions>`）：传给框架后，`options.value = next` 会让框架按新对象重新协调；可运行的例子见 `examples/pages/shared-pair.ts`（「暂停本页」用换对象而不是改字段）。
...
| pnpm build | 通过。生成 `dist/index.js`（22.33 kB，22330 字节）与类型声明（构建后处理改写说明符为 `.js`，并断言产物形态）；按包导入仅得到 defineRefresh、createRefreshManager、useRefresh 三个函数 |
...
| pnpm complexity | 11 个源文件、1883 行、137 个结构分支、最大函数圈复杂度 8 |
...
| pnpm check:docs | 通过。15 项一致性门禁，清单与各项动机以 `scripts/check-docs.mjs` 的自述注释为准；统一文档就在仓库根目录。`pnpm check:docs` 还会打印叶子→测试标题的可追溯性报告（见下） |
