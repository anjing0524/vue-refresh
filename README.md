...
- 框架**不判断参数值是否合法**（那是调用方的责任）：只做稳定编码，沿用 JSON 语义——`-0` 与 `0` 同键，`NaN`／`Infinity` 按 `null`，`undefined` 与函数字段按省略，`Date` 按其 ISO 字符串。传函数、Proxy 这类复制不了的值由 `structuredClone` 拒绝；循环引用让编码交不出身份（内部是 `null`）。两者提交时都按 `rejected` 拒绝（不走 `onError`）。
...
| pnpm build | 通过。生成 `dist/index.js`（10.21 kB，10211 字节）与 5 个声明文件（构建后处理改写说明符为 `.js` 并断言产物形态） |
...
| pnpm complexity | 5 个源文件、872 行、87 个结构分支、最大函数圈复杂度 11 |
...
| 阶段 15 交付产物 | `pnpm build` ＋ `pnpm pack` 生成 `vue-refresh-0.0.0.tgz`：9 个条目 = `dist/` 7 个（`index.js` 10.21 kB，10211 字节、sourcemap、5 个 `.d.ts`）＋ `package.json` ＋ README。最终包 SHA-256 记在[统一文档](./统一刷新管理.md) §5 的 G04 行，不写在这里——README 由 npm 强制打进包内，把包自身哈希写进包内文件没有解 |
...
| TS 4.9 类型消费（G03 阻断节点） | 实测：`typescript@4.9.5` ＋ `module/moduleResolution: Node16` ＋ `strict` ＋ `skipLibCheck` 下 `tsc --noEmit` **本包 5 个声明文件零错误**（含 `@ts-expect-error` 反例：缺字段、Source 不变性、`refresh` 不接受参数、取消分支不带 `reason`）；不再需要状态库，因此没有需要跳过的 peer 校验。Vue 3.5.42 自身的 `.d.ts` 要求 TS ≥5.4，故 `skipLibCheck: true` 是前置条件。确认人 2026-09-16 裁决：声明只限定到本包自身的类型（自 TS 4.9 起可用，已实测） |
...
失败结算与下个周期继续、交付副本独立、参数守卫、回调隔离、销毁、可见性切换的退订与恢复、
运行期失败只走返回值或 `onError`；
