...
固定资源定义（URL ＋ interface） → 提交边界准备参数 → 共享实例执行 → 结果写进结果表 → 每个页面按身份读到同一个对象。

```text
src/public-types.ts   公共类型的唯一代码定义与状态取值常量
src/source.ts         资源声明（带类型的 URL 字符串）、参数准备与稳定键
...
src/vue.ts            组件适配与安装：配置快照、需求、参数准备、生命周期、可见性、结果表接线、按 every 节流
...
- **核心不认识页面**：它认识的是一个身份，加上挂在这个身份下的一组**配置快照**——页面的登记就是它自己那份 `Config`（`enabled` / `every: number | null` / `active`，适配层原地改写），没有任何回调、没有页面名册。参数准备在适配层的提交边界完成，**框架不替调用方跑任何回调**（业务准入由调用方在 `submit` 之前自己判），核心拿到的永远是一份可用身份，因此它不写 `try/catch`、也没有失败通知通道（ADR-64、ADR-66、ADR-74）。`core.ts` 按职责分成六个分段：状态观测与生命周期、页面操作、观测面、身份注册表、后台执行、调度。
...
// `defineRefresh` 只做类型声明：它的返回值就是那个 URL 字符串（`P`／`T` 在这里声明一次）。
...
| pnpm build | 通过。生成 `dist/index.js`（11.18 kB，11178 字节）与 6 个声明文件（构建后处理改写说明符为 `.js` 并断言产物形态；`vue` 与 `pinia` 都是外部依赖） |
...
| pnpm complexity | 6 个源文件、1130 行、77 个结构分支、最大函数圈复杂度 7 |
| node scripts/benchmark.mjs | 24 订阅 / 8 身份 / every=25ms / maxConcurrent=4，3 次 × 2s：取数中位 632 次（收敛比 0.99；按订阅计的反事实 1920 次）、在途峰值 4（框架 `running` 投影峰值同为 4，未超上限）、结束瞬间 8 个共享实例 / 24 份声明、**释放后残留全 0**；事件循环延迟 mean 1.09ms / p99 1.70ms / max 7.27ms。规模可用 `--subscriptions/--identities/--duration/--every/--runs` 改；**不设性能阈值**，只在真实在途超过 `maxConcurrent` 或释放后有残留时以非零码退出 |
| 阶段 15 交付产物 | `pnpm build` ＋ `pnpm pack` 生成 `vue-refresh-0.0.0.tgz`：10 个条目 = `dist/` 8 个（`index.js` 11.18 kB，11178 字节、sourcemap、6 个 `.d.ts`）＋ `package.json` ＋ README。最终包 SHA-256 记在[统一文档](./统一刷新管理.md) §5 的 G04 行，不写在这里——README 由 npm 强制打进包内，把包自身哈希写进包内文件没有解 |
