...
- 全部可变状态都在 `core.ts`，按所属分三处：**跨实例的**挂在协调者上（`identities` 身份键 → 共享实例、`queue`/`running`、唯一唤醒 Timer），**一个身份自己的**在 `Resource` 类里（声明者、未完成的要求、到期与当前执行），**结果**在结果表（`store.ts` 接 Pinia）。页面 → 身份这条映射只在适配层（ADR-66）。
...
**只有「读者」才跟随，而且按本页自己的 `every` 节流**（ADR-67）：这一页**有资格**（声明着它、配置有效、开启、激活且浏览器可见），或它**刚点过一次刷新且此刻可见**时，
...
pnpm build:demo
pnpm test:browser
node scripts/trace-core.mjs  # 差分轨迹：用 8 个脚本化场景驱动核心，打印只看结构的可观测轨迹；改动前后各跑一遍做 cmp，证明「零行为变化」
...
| pnpm test | 通过：46 个用例全绿（`tests/core.test.ts` 35、`tests/vue.test.ts` 11），覆盖 A01–A21（A10「框架上限到期」与 A15 已随能力/入口移除）。核心用例直接驱动 `RefreshCore` 并提供配置快照（含一条「运行期失败只走返回值或结果表这一格」的边界总账）；Vue 用例用无 DOM 的自定义渲染器 ＋ 真实 KeepAlive，安装路径用最小 `document` 替身，不冒充真实可见性测试，并用真实 Timer 验证按 `every` 节流。**ADR-67（读取面改为 `updatedAt` 时间差节流）落地后待重跑**：`tests/vue.test.ts` 的 A21 用例改为节流语义并新增「写端稀于本页 every 时写入即抄」一条（44 → 45） |
| pnpm build | 通过。生成 `dist/index.js`（11.60 kB，11600 字节）与 6 个声明文件（构建后处理改写说明符为 `.js` 并断言产物形态；`vue` 与 `pinia` 都是外部依赖） |
...
| pnpm complexity | 6 个源文件、1139 行、82 个结构分支、最大函数圈复杂度 8 |
| node scripts/benchmark.mjs | 24 订阅 / 8 身份 / every=25ms / maxConcurrent=4，3 次 × 2s：取数中位 634 次（收敛比 0.99；按订阅计的反事实 1920 次）、在途峰值 4（框架 `running` 投影峰值同为 4，未超上限）、结束瞬间 8 个共享实例 / 24 份声明、**释放后残留全 0**；事件循环延迟 mean 1.09ms / p99 1.77ms / max 7.28ms。规模可用 `--subscriptions/--identities/--duration/--every/--runs` 改；**不设性能阈值**，只在真实在途超过 `maxConcurrent` 或释放后有残留时以非零码退出 |
| 阶段 15 交付产物 | `pnpm build` ＋ `pnpm pack` 生成 `vue-refresh-0.0.0.tgz`：10 个条目 = `dist/` 8 个（`index.js` 11.60 kB，11600 字节、sourcemap、6 个 `.d.ts`）＋ `package.json` ＋ README。最终包 SHA-256 记在[统一文档](./统一刷新管理.md) §5 的 G04 行，不写在这里——README 由 npm 强制打进包内，把包自身哈希写进包内文件没有解 |
