...
| tests/core.test.ts、tests/vue.test.ts | 51 个现行契约自动测试通过；含真实 Vue/KeepAlive/Pinia 和两个 SSR 渲染。定制渲染器测试不冒充真实浏览器可见性测试 |
