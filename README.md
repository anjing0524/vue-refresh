...
新身份的第一份内容、本页显式 `refresh()` 那一次、以及重新成为读者后的第一份内容**不等窗口**（此时这一页没有「上次读取时间」，没有时间就直接读），立即上屏。快页面（`every` 小）因此比慢页面跳得密，慢页面主动要的就是这个；
...
**一处口径（明说）**：暂停页**还没有读到过**时会跟着第一份到达的数据上屏一次（此后它有了「上次读取时间」就冻住）——「没有时间就直接读」不再区分「第一次」与「点过刷新」；而失败那一笔不改数据时间、只改失败位，靠「同一版」这一条判定照样进画面，不会漏掉。
...
| pnpm build | 通过。生成 `dist/index.js`（11.67 kB，11668 字节）与 6 个声明文件（构建后处理改写说明符为 `.js` 并断言产物形态；`vue` 与 `pinia` 都是外部依赖） |
...
| pnpm complexity | 6 个源文件、1152 行、82 个结构分支、最大函数圈复杂度 7 |
...
| 阶段 15 交付产物 | `pnpm build` ＋ `pnpm pack` 生成 `vue-refresh-0.0.0.tgz`：10 个条目 = `dist/` 8 个（`index.js` 11.67 kB，11668 字节、sourcemap、6 个 `.d.ts`）＋ `package.json` ＋ README。最终包 SHA-256 记在[统一文档](./统一刷新管理.md) §5 的 G04 行，不写在这里——README 由 npm 强制打进包内，把包自身哈希写进包内文件没有解 |
