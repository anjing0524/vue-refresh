...
src/vue.ts            组件适配与安装：配置快照、需求、参数准备与校验、生命周期、可见性、结果表接线、按 every 节流
...
- **核心不认识页面**：它认识的是一个身份，加上挂在这个身份下的一组**配置快照**——页面的登记就是它自己那份 `Config`（`enabled` / `every: number | null` / `active`，适配层原地改写），没有任何回调、没有页面名册。参数准备与 `validate` 在适配层的提交边界完成，核心拿到的永远是一份可用身份，因此它不写 `try/catch`、也没有失败通知通道（ADR-64、ADR-66）。`core.ts` 按职责分成六个分段：状态观测与生命周期、页面操作、观测面、身份注册表、后台执行、调度。
- 全部可变状态都在 `core.ts`，按所属分三处：**跨实例的**挂在协调者上（身份键 → 共享实例、队列与并发槽、唯一唤醒 Timer），**一个身份自己的**在 `Resource` 类里（声明者、这一轮的结果产出了没有、到期与当前执行），**结果**在结果表（`store.ts` 接 Pinia）。页面 → 身份这条映射只在适配层（ADR-66）。
  **声明与资格分开**：页面挂载期间一直声明着身份（暂停、失活、隐藏都不撤销），资格（开启 ＋ 激活 ＋ 浏览器可见）现算，只决定要不要取数。声明侧只有一处事实（挂在共享实例上的那份成员资格），没有镜像字段。
...
关闭 `enabled` 只失去资格（不再自动取数、画面冻结），不撤销声明也不删结果；**框架不留「欠一次」的账**——
刷新就是当下那句命令：没有执行就排到最前，有执行就用这一轮的结果，**暂停页仍能刷新一次**（那一次会立刻上屏）。
...
**只有「读者」才跟随，而且按本页自己的 `every` 节流**（ADR-67）：这一页**有资格**（声明着它、配置有效、开启、激活且浏览器可见），或它**点过一次刷新、还没看到那一拍且此刻浏览器可见**时，
每次写入触发判定——新格的 `updatedAt` 距展示中那份满一个 `every` 才换画面；节流窗口内的新版本不改变画面，写端稀于 `every` 时写入即抄。
新身份的第一份内容、本页显式 `refresh()` 那一次、以及失活/暂停恢复后的第一份内容**不等节流**，立即上屏。快页面（`every` 小）因此比慢页面跳得密，慢页面主动要的就是这个；
取数仍按同一身份所有页面里**最小的 `every`** 进行，慢页面是白嫖快页面的结果，不额外发请求。
暂停、失活、隐藏后**画面冻结在最后一帧**；暂停页自己按一次「刷新」仍会更新（框架记下「点过一次刷新、还没看到那一拍」，下一份一到就抄），
而**失活或隐藏期间**才按「刷新」的什么都不产生；**失活之前点过刷新**、浏览器仍可见的缓存页会在后台更新那一帧（ADR-67 的角落，确认人裁决「允许它更新一帧呗」）。
...
| pnpm test | 通过：49 个用例全绿（`tests/core.test.ts` 36、`tests/vue.test.ts` 13），覆盖 A01–A21（A10「框架上限到期」与 A15 已随能力/入口移除）。核心用例直接驱动 `RefreshCore` 并提供配置快照（含一条「运行期失败只走返回值或结果表这一格」的边界总账，以及两条钉住「刷新是给身份的命令、同一轮内合并、页面自己循环就是页面自己的循环」的用例）；Vue 用例用无 DOM 的自定义渲染器 ＋ 真实 KeepAlive，安装路径用最小 `document` 替身，不冒充真实可见性测试，并用真实 Timer 验证按 `every` 节流与「没点过刷新的暂停页什么都不上屏」 |
| pnpm build | 通过。生成 `dist/index.js`（11.56 kB，11557 字节）与 6 个声明文件（构建后处理改写说明符为 `.js` 并断言产物形态；`vue` 与 `pinia` 都是外部依赖） |
...
| pnpm complexity | 6 个源文件、1155 行、81 个结构分支、最大函数圈复杂度 7 |
| node scripts/benchmark.mjs | 24 订阅 / 8 身份 / every=25ms / maxConcurrent=4，3 次 × 2s：取数中位 637 次（收敛比 1；按订阅计的反事实 1920 次）、在途峰值 4（框架 `running` 投影峰值同为 4，未超上限）、结束瞬间 8 个共享实例 / 24 份声明、**释放后残留全 0**；事件循环延迟 mean 1.09ms / p99 1.80ms / max 10.47ms。规模可用 `--subscriptions/--identities/--duration/--every/--runs` 改；**不设性能阈值**，只在真实在途超过 `maxConcurrent` 或释放后有残留时以非零码退出 |
| 阶段 15 交付产物 | `pnpm build` ＋ `pnpm pack` 生成 `vue-refresh-0.0.0.tgz`：10 个条目 = `dist/` 8 个（`index.js` 11.56 kB，11557 字节、sourcemap、6 个 `.d.ts`）＋ `package.json` ＋ README。最终包 SHA-256 记在[统一文档](./统一刷新管理.md) §5 的 G04 行，不写在这里——README 由 npm 强制打进包内，把包自身哈希写进包内文件没有解 |
...
核心测试覆盖声明幂等与校验失败不改状态、共享身份、恢复与不补跑、并发上限与排队次序、迟到结束无效、
