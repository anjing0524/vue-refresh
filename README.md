...
| pnpm build | 通过。生成 `dist/index.js`（20.96 kB / gzip 6.46 kB）与类型声明（构建后处理改写说明符为 `.js`，并断言产物形态）；按包导入仅得到 defineRefresh、createRefreshManager、useRefresh 三个函数 |
...
| pnpm complexity | 11 个源文件、1755 行、131 个结构分支、最大函数圈复杂度 8 |
