...
根为对象，嵌套值为 `null` / `boolean` / `string` / 有限 number（不含 `-0`）/ 普通记录 / 数组。
调用方不传 Proxy、访问器、隐藏字段或特殊容器；框架不承诺对这些违约输入逐类探测。

提交边界**一次**执行，且只有一趟结构遍历：

```text
structuredClone → 递归：值域守卫 → 深度守卫 → 按需冻结 → 稳定编码
...
- 参数提交：复制一次、一趟遍历（守卫/冻结/编码）一次、可选业务校验一次。按值展开参数规模记 K，成本 `O(K)`。
