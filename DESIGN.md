...
- **三个不等节流的边沿**：身份落定（`submit` 被接纳）、重新成为读者（配置/生命周期 watcher）、显式 `refresh()`（`pending` 放行）。
  首查结果、失活恢复、手动刷新因此都不会被节流窗口拖后；其余版本按 `updatedAt` 时间差抄。
  **前两个边沿只在有资格时生效**（它们置起的是 `immediate`，读闸门仍要资格或「点过一次刷新」才放行）。
...
- **适配层三个标志**：`released`（`onScopeDispose` 置位，释放后 `submit` 返回 `cancelled`、`refresh` 无副作用）、
  `pending`（点过一次刷新、还没看到那一拍：`refresh` 被接纳时置起，抄到一版时清掉）与 `immediate`
  （边沿欠一份：下一份内容不等节流，且**只在有资格时**——身份落定、重新成为读者）。它们都在适配层，核心不认识。
  **`pending` 与节流基线是两件事**：基线（`sampled`）记已经抄到的那一版，`pending` 记还欠一次抄写——把基线置空
  顶替 `pending` 会被一次不产出的副作用运行当场用掉（ADR-70 记了这次实测）。
- **读闸门在适配层**：核心不再回答「谁该跟随结果表」，只提供两个只读判定——`isEligible`（这一页有没有取数资格）
  ...
  `immediate` **不能**拿来当第二项：它在配置与生命周期边沿也会置起，那样从未上过屏的暂停页会在下一份结果到达时当场显示。
