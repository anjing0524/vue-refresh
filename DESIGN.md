...
- **三处「没有读取时间就直接读」**：身份落定（`submit` 真的换了身份；同身份重复 `submit` 是幂等的，不动这个基准）、重新成为读者（配置/生命周期 watcher）、显式
  ...
  **配置变化不叫醒读取面（ADR-73）**：唤醒那个副作用的只有两件事——**这一格被写入**、**这一页换了身份**。
  watcher 只做两件与此有关的事：**在资格从无到有那一条边上**把 `lastReadAt` 清掉并唤醒读取面一次（于是它立刻读回该身份
  当前那一版、不等窗口），以及让核心重算调度——同一份配置内的变化（改频率、改可见性）只重排调度，不动基准（ADR-91、ADR-94）。
  因此「重新成为读者」与另外两处的差别是：首查与手动刷新本身就是写入触发的，而重新成为读者由资格边沿**主动唤醒**一次（读的是表里当前那一版，不是等写入
