...
- 重新显示时：画面**保持**已有结果（不重新取数）并按间隔调度；实例已销毁则重建并首查。读面只在**资格从无到有**那一条边上重置读取基准。
...
- **三处「没有读取时间就直接读」**：身份落定（`submit` 真的换了身份；同身份重复 `submit` 是幂等的，不动这个基准）、重新成为读者（配置/生命周期 watcher）、显式
  ...
  **配置变化不叫醒读取面（ADR-73）**：唤醒那个副作用的只有两件事——**这一格被写入**、**这一页换了身份**。
  watcher 只做两件与此有关的事：**在资格从无到有那一条边上**把 `lastReadAt` 清掉（于是下一份写入不受窗口限制），
  以及让核心重算调度——同一份配置内的变化（改频率、改可见性）只重排调度，不动基准（ADR-91）。
