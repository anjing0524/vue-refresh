...
本节是**内部术语、字段、所有权与算法**的唯一维护入口；统一文档只保留对外含义与指针。
...
- 版本域只表达**登记顺序**；「在本次动作之后**启动**」还需要启动事实——`refreshFloor` 因此读
  `Scheduler.isRunning(task)`：排队未启动的任务取它的版本（可以满足本次刷新），已在执行的任务取
  「版本＋1」（必须等下一个任务）。只用版本比较会把在途任务误当成已满足，等于拿点击之前发出的数据
  冒充新数据。
