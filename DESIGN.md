...
**这份文档是什么**：实现侧的**架构 ＋ 详细设计**。§1–§2 是架构（模块划分、依赖方向、模块职责、数据流、调用链），
§3–§7 是详细设计（数据模型与字段、参数与结果边界、调度与并发竞态、生命周期与 Vue 适配、不变量与顺序约束），
§8 是验证入口，§9 是读码索引（附录）。

**它不写什么**：对外承诺（行为、取值、判据）的正式定义在 [统一刷新管理文档](./统一刷新管理.md)；
取舍理由、被否方案与历史在 [ADR.md](./ADR.md)。本文只写**结构与算法**——字段与符号一律用表、一行说清，
不复述理由、也不重抄条文；两者冲突时以《统一刷新管理文档》§3／§4 为准。

**读代码从 §9 开始**：三条进入路线、读之前先记住的七个词、每个符号的一句话作用，以及「遇到 `if` 时它在防什么」的对照表。
`src/core.ts` 的五个 `═══` 分段与 §2.1 的分工一一对应（`pnpm check:docs` 第 11 组断言两边的名字与顺序一致）。
...
| `ResultCell` | `data` / `updatedAt` / `error` / `failedAt` | 四个平字段 | 取值与含义见 §3.8 |
| RefreshCore | `identities` / `queue` / `running` / `wakeup` / `flushing` / `disposed` | 身份键 → 实例（`Map`）／待取顺序（数组）／在途集合（`Set`）／唤醒句柄或 `null`／两个布尔位 | 全部 `private`：外部只能走命名操作（`isDisposed` / `setConfig` / `submit` / `refresh` / `isEligible` / `readResult` / `undeclare` / `dispose`）；核心私有的动作与收尾顺序见 §5、§3.10 |
...
拿掉它就无法同时满足 A14 与 A12（ADR-70 记了试过并否掉的两条路）。
...
主干五步、四个判定；`refresh` 的三条分支不在图上，见 §3.6 的表。
...
flowchart TD
  ...
  Current -->|"是"| Write["核心写表 → 同步唤醒适配层读取面（§6.5）"]
...
**发布的同步性**：页面 watcher 可能在发布当中换身份或卸载，本轮快照随即作废，并由一次微任务强制重新唤醒读取面（§6.5）。
...
- **没有业务回调接口**：提交边界只做参数准备（复制／值域／编码），没有 `validate` 这类准入回调；业务准入由调用方在 `submit` 之前自己判，
  那里的抛错就地变成同步 `rejected`。框架自身不写诊断日志（ADR-64、ADR-74）。历史：交付回调随结果表删除（ADR-59）、
  失败通知回调随失败进结果表删除（ADR-63）、`cleanup` 随 ADR-64 删除。
...
- **读闸门在适配层**：`core.isEligible(config, url, key) || (lastReadAt === null && !document.hidden)`，两个出口共用这一个闸门。
  ...
  在 `refresh` 的入口闸那一刻已经判过，之后失活仍允许它更新那一帧（§6.2 的角落）。失败出口与数据出口共用闸门，但**没有窗口**——
...
| 符号 | 做什么 | 明细 |
|---|---|---|
| `useRefresh` | 组件侧唯一入口：建本页配置槽、跟踪配置与生命周期，返回 `RefreshHandle`；参数准备在提交边界完成 | §6.1、§6.2 |
| `createRefreshManager` | 应用级协调者：接结果表（`pinia`）、取数实例（`axios`）与并发上限；`install` 再接可见性监听与卸载释放 | §6.3 |
| `Config` | 一页在核心里的登记：`{ enabled, every, present }`，适配层原地写、核心只读；这份登记就是声明 | §3.3 |
| `RefreshFailure` | 失败出口的形状：`{ error, failedAt }`；`null` ＝ 自最后一次成功以来没失败过（含从未失败） | §3.3 |
| `handle.failure` | 本页看到的最近一次失败：与 `display` 共用读闸门，但不参与数据窗口、换一笔就发布 | §6.5 |
| 协调者（`RefreshCore`） | 身份注册表、唯一唤醒 Timer、有序队列、并发槽；公开面只有 §3.3 那一行列出的 8 个命名操作 | §3.3、§5 |
| `RefreshCore.setConfig` | 写这一页配置槽的三个值（非法只把 `every` 置 `null`）并重算一次到期与唤醒 | §6.1 |
| `RefreshCore.submit` | 声明或更新身份：`resourceOf` 扫描 → 摘旧 → 挂新；相同身份幂等 | §3.4、§5 |
| `RefreshCore.refresh` | 给身份下一句「现在再取一次」，返回「这句命令收下了没有」；三条分支 | §3.6 |
| `RefreshCore.isEligible` | 这一页此刻有没有取数资格（现算）；核心只回答这一问，不回答「谁该跟随」 | §3.5 |
| `RefreshCore.readResult` | 读这一格——结果表对核心的唯一读缝，适配层读取面用它 | §6.5 |
| `RefreshCore.undeclare` / `RefreshCore.dispose` | 撤销一页的声明（没有声明者就地回收）／销毁（幂等、不可复用） | §3.4、§6.3 |
| `RefreshCore` 私有动作 | `resourceOf` 扫描定位／`resourceFor` 查建实例／`releaseIfUnused` 回收／`enqueue`・`dequeue` 进出队／`enqueueDue` 到期入队／`startQueued` 占槽启动／`run` 收尾顺序的唯一处 | §5.1、§3.10 |
| `RefreshCore.flush` | 一次合并调度：取消旧 Timer → 到期入队并收齐最早到期时刻 → 按队列次序占槽启动 → 队列空了安排唯一唤醒 | §5.1 |
| `Resource` | 一个身份自己的状态与判定：`dueAt` / `eligibleEvery` / `isPresent` / `isEligible` / `isCurrent` / `hasExecution` / `isWanted` / `settle(at)`；不持有核心 | §3.3、§5.3 |
| `prepareParameters` | 提交边界只执行一次：复制 → 值域检查 → 编码身份键 | §4.1 |
| `useRefreshStore` | 结果表：`write` / `fail` / `remove`（写入端）、`read`（核心读取面的底层）、`list` / `size`（观测面） | §3.3、§4.2 |
| 观测投影 `snapshot` | 只读计数投影（`resources` / `results` / `queued` / `running`），住在开发侧 `scripts/observe.ts`，不是包契约 | §4.2 |
