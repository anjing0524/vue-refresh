/**
 * 集成验证台：`tests/browser.html` 与 Playwright 场景共用的受控视图（`?mode=controlled` 手动结算 ＋ 只读测试桥）。
 * 与三条代表页面不是同一类东西，因此独立成文件；它的请求函数也不能复用 `sources.ts` 的 `runQuote`
 * ——手动结算只在这里需要。
 */
...
export function mountHarness(): void {
  ...
  const refresh = createRefreshManager({ maxConcurrent: params.get('slots') === '1' ? 1 : 2, http, pinia })
  app.use(pinia)
  app.use(refresh)
