...
export const QueryListPage = defineComponent({
  ...
  setup() {
    ...
    // 页面自己记「上一次手刷的点击时刻」：框架不交付「这次是谁触发的」，这是页面侧的事实。
    // 从未手刷过时为正无穷，与结果时间比较恒为假。
    const manualAt = ref(Number.POSITIVE_INFINITY)
    ...
    // 默认 flush 是 pre 且首次不触发；成功会把失败位清回 null，所以只在非空的一笔上计数。
    ...
    const once = (): void => {
      ...
      task.submit(args)
      // 手刷没有回执：页面自己记下点击时刻，交付后比较 `updatedAt` 判断来源。
      manualAt.value = Date.now()
      task.refresh()
    }
    ...
    return () => {
      // 首查就失败时 display 不是 null，而是 `data: null` ＋ `failedAt` 有值：空态按 data 判。
