...
import type { QueryResult, RefreshDisplay, RefreshError } from './public-types.ts'

/**
 * 内部模型：核心拥有的运行时事实。
 *
 * 只保存必须保存的事实，不放派生值：Task 的执行位置由 queue/running 决定；
 * 最短间隔由各 Subscription.every 现算；资格由配置快照、生命周期、可见性和
 * 当前活动推导。字段的写入者与释放时机见统一文档「数据模型」一节。
 */
 ...
 * 句柄当前活动的判别：独立查询或共享订阅。
 *
 * 只在核心内部使用：`QueryRun` / `Subscription` 都不由包导出，消费方观察不到 `kind`。
 */
...
/** 一次独立查询的取消与结算句柄；用对象身份判断是否仍然有效。 */
export interface QueryRun {
  readonly kind: typeof ActivityKind.Query
  readonly controller: AbortController
  /** 原生 Promise 的 resolver：只有第一次结算生效，因此不需要 settled 镜像。 */
  readonly settle: (result: QueryResult) => void
}

/** 页面与共享 Resource 的一次连接；退出即失效。 */
export interface Subscription {
  readonly kind: typeof ActivityKind.Subscription
...
/** 同一时刻只能有一个活动：独立查询或共享订阅。 */
export type Activity = QueryRun | Subscription

/** 交付门槛：必须等到指定 Resource 生存期内不低于该版本的结果。 */
...
/**
 * Submission 的交付要求，只有两种（外加「没有要求」）：
 * - `{ barrier: null }`：本次显式操作尚欠一个新后台请求，登记任务时填入具体门槛；
 * - `{ barrier }`：请求已登记，等待不低于该版本的结果；
 * - `null`：没有要求，可交付已有合格结果。
 * 不再用字符串哨兵：两种「尚欠请求」的区别就是是否已登记资源，直接由 `barrier` 是否为 null 表达。
 */
export type Delivery = DeliveryRequest | null

/** 尚欠一次合格请求时的要求；`barrier` 为 null 表示尚未登记资源。 */
export interface DeliveryRequest {
  readonly barrier: DeliveryBarrier | null
}

/** 句柄的可恢复参数与交付要求；临时退出保留，新操作清除。 */
...
export interface Handle {
  ...
  /** 作用域释放回调；至多一个，卸载时执行。 */
  cleanup: (() => void) | null
  /** 页面显式操作号；用于识别「本次操作是否仍被接纳」。 */
  operationId: number
  /** 当前可恢复参数；没有有效提交时为 null。 */
  submission: Submission | null
  /** 当前活动；query 与 subscription 互斥。 */
...
/** 同一定义、同一组参数值的一次共享生存期；最后一个订阅退出即销毁。 */
export interface Resource {
  ...
  nextVersion: number
  /** 至多一个当前后台任务；旧任务被替换后不再等于 resource.task。 */
  task: Task | null
 ...
 * 独立查询执行路径看到的编排层窄端口。
 *
 * `query.ts` 只依赖这四项事实，不依赖 `Manager` 的完整形状：`Manager` 在结构上满足它，
 * 因此不需要转发类或适配层，依赖方向也不必为它破例。
 */
export interface QueryHost {
  /** Manager 是否已销毁；查询的有效性判据之一。 */
  isDisposed(): boolean
  readonly clock: Clock
  /** 只解除活动登记，不结算、不取消、不安排调度。 */
...
export interface ScheduleHost {
  ...
  /** 为 Resource 登记一次后台执行（分配版本、建立交付门槛）。 */
