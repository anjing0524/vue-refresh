...
const CASES = [
  ...
  [11, 'DESIGN.md', '状态观测与生命周期、页面操作、身份注册表、后台执行、调度', '状态观测与生命周期、页面操作、身份注册表、调度、后台执行', 'core.ts 五分段的名字与顺序两边一致'],
  [12, '统一刷新管理.md', '键排序', '键的排序', '主题引用的锚点必须仍带那个主题（跨多行条文）'],
  [13, '统一刷新管理.md', '| 环境允许 |', '| 环境允许X |', '§0 的词必须是正文真的在用的词'],
  [14, 'README.md', '`index.js` 10.71 kB，10710 字节', '`index.js` 10.71 kB，10711 字节', 'README 记的产物体积必须等于实测'],
  [15, 'DESIGN.md', '一条结果只写一格', '一条结果只写一格（见 `GhostSymbol`）', '文档提到的代码符号必须真在 src 里'],
  [16, 'README.md', '通过：68 个用例全绿', '通过：67 个用例全绿', '写死的用例总数必须等于实测'],
